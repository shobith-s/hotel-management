...
export default function TopBar({ title }: TopBarProps) {
  ...
  const allowed           = user ? (ROLE_ALLOWED_PATHS[user.role] ?? []) : []
  const canAccessSettings = allowed.includes('/settings')

  return (
    <>
      <header className="bg-surface/80 backdrop-blur-xl sticky top-0 w-full z-40 shadow-header h-20 flex items-center justify-between px-10">
        {title ? (
            ...
            <div className="relative">
              ...
              {dropdownOpen && (
                <>
                  ...
                  {/* Dropdown panel */}
                  <div className="absolute right-0 top-full mt-2 z-50 w-52 bg-surface rounded-2xl shadow-xl border border-outline-variant/20 overflow-hidden">
                    <div className="px-4 py-3 bg-surface-container-low/60">
                    ...
                    <div className="py-1">
                      ...
                      <button
                        ...
                        className="w-full px-4 py-2.5 text-left text-sm text-error hover:bg-error/5 transition-colors flex items-center gap-3"
                      >
